...
import static com.bale.constant.CodeGenConstant.*;
...
public class JavaxRsServerCodeGenerator extends DefaultGenerator {
    ...
    private void generatePathEntity(String key, Path path) {
        ...
        final PathEntity pathEntity = PathEntity.builder()
                .apiPackage(codeGenCfg.apiPackage())
                .servicePackage(codeGenCfg.servicePackage())
                .className(className)
                .urlPath(splitKeys[0])
                ...
                .pathPutEntity(getPathOperationProps(path.getPut(), methodPath))
    ...
    private List<ModelPropEntity> getModelProps(Model model) {
        JavaxRsServerCodeGen codeGenCfg = (JavaxRsServerCodeGen) config;
        return model.getProperties().entrySet()
                .stream()
                .map(entrySet -> ModelPropEntity.builder()
                        .paramName(entrySet.getKey())
                        .modelParamDesc(ModelParamDescEntity.builder()
                                .required(entrySet.getValue().getRequired())
                                .type(codeGenCfg.getSwaggerType(entrySet.getValue()))
                                .build())
                        .build())
    ...
    private PathOperationEntity getPathOperationProps(Operation operation, String urlPath) {
        return Optional.ofNullable(operation)
                .map(opera -> PathOperationEntity.builder()
                        .methodName(opera.getOperationId())
                        .urlPath(Objects.nonNull(urlPath) ? "/{" + urlPath : null)
                        .responseType(getResponseType(opera.getResponses()))
                        .params(getPathParams(opera.getParameters()))
                        .build())
                .orElse(null);
    }
    ...
    private List<PathParamsEntity> getPathParams(List<Parameter> params) {
        ...
        return params.stream()
                .map(param -> PathParamsEntity.builder()
    ...
    private String getResponseType(Map<String, Response> responseMap) {
        Optional<Map.Entry<String, Response>> entryOptional = responseMap.entrySet()
                .stream()
                .filter(entrySet -> entrySet.getKey().equals(String.valueOf(HttpStatus.SC_OK)))
                .findAny();

        final Property scheme = entryOptional.map(Map.Entry::getValue)
                .map(Response::getSchema)
                .orElse(new ObjectProperty());
        ...
        if (scheme.getType().equals("ref")) {
            ...
            apiImportClassList.add(ImportClassEntity.builder()
                    .className(modelNameMap.get(className))
                    .build());
            return className;
        } else {
            JavaxRsServerCodeGen codeGenCfg = (JavaxRsServerCodeGen) config;
    ...
    private String getOutputFilePath(String fileName, String templateName) {
        ...
        if (API_TEMPLATE_FILE.equals(templateName)) {
            return codeGenCfg.outputFolder() + File.separator
                    + codeGenCfg.apiPackage().replaceAll("\\.", Matcher.quoteReplacement(File.separator))
                    + File.separator + fileName + ".java";
        } else if (MODEL_TEMPLATE_FILE.equals(templateName)) {
            return codeGenCfg.outputFolder() + File.separator
                    + codeGenCfg.modelPackage().replaceAll("\\.", Matcher.quoteReplacement(File.separator))
                    + File.separator + fileName + ".java";
        } else {
            return codeGenCfg.outputFolder() + File.separator
                    + codeGenCfg.servicePackage().replaceAll("\\.", Matcher.quoteReplacement(File.separator))
                    + File.separator + fileName + ".java";
        }
    }
